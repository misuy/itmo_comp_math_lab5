...
fun main(args: Array<String>) {
    ...
    val stirlingPolynomial: Function? = if (nodes.dots.count() % 2 != 0) buildStirlingPolynomial(nodes) else null;
    if (stirlingPolynomial == null) println("Полином Стирлинга не построен, т.к. число узлов четно");
    else chartPanel.addFunction(FunctionGraph(stirlingPolynomial, Color.ORANGE));
    ...
    while (flag) {
        ...
        try {
            ...
            if (besselPolynomial != null) println("Значение полинома Стирлинга: ${besselPolynomial.getValue(variables)};");
        }
